...
def makeLineFace_orig(edge, toolRadius):
    """makeLineFace_orig(edge, toolRadius) ..."""
...
def wiresToPathFace(wires, toolDiameter):
    ...
    return combineRegions(allFaces)


def wiresToPathFace3D(wires, toolDiameter):
    td = round(toolDiameter * 0.9995, 6)
    toolRadius = td / 2.0
    # print("wiresToPathFace() toolRadius: {}".format(toolRadius))
    allFaces = []
    ...
    if len(allFaces) == 1:
        reg = allFaces[0]
        reg.translate(
            FreeCAD.Vector(0.0, 0.0, wires[0].BoundBox.ZMin - reg.BoundBox.ZMin)
        )
        return reg

    region = combineRegions(allFaces)
    region.translate(
        FreeCAD.Vector(0.0, 0.0, wires[0].BoundBox.ZMin - region.BoundBox.ZMin)
    )
    return region


def combineRegions(regions):
    comp = Part.makeCompound(regions)
