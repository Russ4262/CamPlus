...
if FreeCAD.GuiUp:
    import freecad.camplus.boundary.BoundaryShapeGui as BoundaryShapeGui
...
class DressupPathBoundary(object):
    def __init__(self, obj, base, job):
        ...
        obj.Inside = True
        if FreeCAD.GuiUp:
            obj.Stock = BoundaryShapeGui.Create(obj)
        else:
            obj.Stock = BoundaryShape.Create(obj)
    ...
    def execute(self, obj):
        # print("DressupPathBoundary().execute()")
...
class PathBoundary:
    ...
    def __init__(self, baseOp, boundaryShape, inside=True):
        ...
        # if isinstance(self.boundary, Part.Shape):
        #    print("PathBoundary().__init__() self.boundary is Part.Shape")
        # if len(self.boundary.Edges) > 0:
        #    print("PathBoundary().__init__() self.boundary.Edges")
    ...
    def execute(self):
        ...
        for cmd in path.Commands[1:]:
            if cmd.Name in Path.Geom.CmdMoveAll:
                ...
                if (
                    edge
                    and len(edge.Edges) > 0
                    and self.boundary
                    and len(self.boundary.Edges) > 0
                ):
