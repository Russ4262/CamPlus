...
def _addBody(doc):
    ...
    body.addProperty(
        ...
        QT_TRANSLATE_NOOP(
            "App::Property",
            "Purpose of Body object.",
        ),
    )
    body.Purpose = "DressupBoundary"
...
def createBoundaryBodyForJob(job, initializeWithSquare=True):
    ...
    body = _addBody(doc)
    body.Placement.Base = App.Vector(0.0, 0.0, job.Stock.Shape.BoundBox.ZMin)
...
def createBoundaryBody(initializeWithSquare=True):
    doc = App.ActiveDocument
    body = _addBody(doc)
    sketch = _addSketchToBody(body)
    if initializeWithSquare:
