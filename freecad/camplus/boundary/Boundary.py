...
import freecad.camplus.boundary.BoundaryUtils as BoundaryUtils
...
class DressupPathBoundary(object):
    ...
    def __init__(self, obj, base, job):
        ...
        obj.Stock = BoundaryUtils.createBoundaryBodyForJob(job, True)
    ...
    def onDelete(self, obj, args):
        ...
        if hasattr(obj, "Stock") and obj.Stock:
            BoundaryUtils.clearBoundaryGroup(obj.Stock)
    ...
    def execute(self, obj):
        ...
        if not hasattr(obj.Stock, "Shape") or obj.Stock.Shape is None:
            ...
            obj.Path = Path.Path([])
            return
        if len(obj.Stock.Shape.Edges) > 0:
            pb = PathBoundary(obj.Base, obj.Stock.Shape, obj.Inside, obj.KeepToolDown)
            obj.Path = pb.execute()
        else:
            Path.Log.error("Boundary object has not shape. Check Boundary sketch and pad.")
